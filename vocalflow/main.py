...
class CrossPlatformOutput:
    ...
    def __init__(self, config: OutputConfig):
        ...
        self.method = (
            self._detect_best_method() if config.method == "auto" else config.method
        )
        ...
        if IS_WINDOWS:
            ...
            try:
                result = subprocess.run(
                    ["which", "xdotool"], check=True, capture_output=True, text=True
                )
                return "xdotool"
    ...
    def start(self):
        ...
        try:
            ...
            self.processing_thread = threading.Thread(
                target=self._process_audio_stream, daemon=True, name="AudioProcessor"
            )
        ...
        while self.is_running:
            try:
                ...
                speech_threshold = max(
                    self.profile.speech_threshold, self.profile.background_noise * 2.5
                )
                ...
                if rms > speech_threshold:
                    ...
                    if (
                        has_speech
                        and silence_start
                        and (current_time - silence_start)
                        > self.profile.avg_pause_length
                    ):
            ...
            for text in texts:
                ...
                if self.filter.is_good_text(text):
                    ...
                    if (
                        ...
                        and self.linguistic_bridge
                        and self.command_agent
                    ):
                        is_command, command_type, metadata = (
                            self.linguistic_bridge.process_command(corrected_text)
                        )

                        if is_command:
                            # Process as command
                            self.logger.info(
                                f"Command detected: '{command_type}' from text: '{corrected_text}'"
                            )
                            confidence = metadata.get("confidence", 0)
                            method = metadata.get("method", "unknown")
                            print(
                                f"🎯 Command: {command_type} (confidence: {confidence:.2f}, method: {method})"
                            )

                            # Execute command using enhanced agent
                            try:
                                success = self.command_agent.process_text_enhanced(
                                    corrected_text
                                )
                                if success:
                                    self.logger.info(
                                        f"Command '{command_type}' executed successfully"
                                    )
                                else:
                                    self.logger.warning(
                                ...
                                self.logger.error(
                                    f"Command execution error: {cmd_error}"
                                )
...
Examples:
  ...
  python -m vocalflow.main --disable-commands # Disable voice commands
    ...
    parser.add_argument(
        "--list-profiles", action="store_true", help="List all available voice profiles"
    )
        ...
        if profiles:
            ...
            for profile_name in profiles:
                ...
                if profile:
                    ...
                    print(
                        f"  {profile_name}: {sessions} sessions, {words} words, last used: {last_used}"
                    )
        else:
