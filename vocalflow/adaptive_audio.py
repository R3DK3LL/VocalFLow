...
class EnvironmentDiscovery:
    ...
    def __init__(self):
        ...
        self.discovery_samples = deque(maxlen=2000)  # Large sample for environment characterization
    ...
    def _check_discovery_completion(self):
        ...
        if (discovery_duration >= self.min_discovery_duration and 
            len(self.discovery_samples) >= 500):
    ...
    def _complete_discovery(self):
        ...
        self.baseline_noise_floor = np.percentile(samples, 10)  # Bottom 10% as noise floor
        ...
        self.typical_speech_range = (self.baseline_noise_floor + dynamic_range * 0.3,
        ...
        self.dynamic_min_threshold = self.baseline_noise_floor * 0.8  # Just below noise floor
        ...
        self.adaptive_silence_duration = 0.8 + (1.2 * stability_factor)  # 0.8 to 2.0 seconds
        
        self.is_discovery_complete = True
        
        self.logger.info(f"Environment discovered: noise_floor={self.baseline_noise_floor:.6f}, "
...
class RelativeSignalProcessor:
    ...
    def update_signal_profile(self, audio_chunk: np.ndarray, is_likely_speech: bool):
        ...
        if len(self.recent_quiet_levels) >= 10:
            self.current_noise_estimate = np.percentile(list(self.recent_quiet_levels), 75)
        
        # Update signal baseline if we have active levels
        if len(self.recent_active_levels) >= 5:
            self.current_signal_baseline = np.percentile(list(self.recent_active_levels), 25)
    ...
    def enhance_signal_relatively(self, audio_chunk: np.ndarray) -> np.ndarray:
        ...
        self.dc_offset = (1 - adaptation_rate) * self.dc_offset + adaptation_rate * current_dc
    ...
    def calculate_relative_quality(self, audio_chunk: np.ndarray) -> float:
        ...
        if self.signal_quality_history:
            consistency = 1.0 - abs(base_quality - np.mean(list(self.signal_quality_history)[-5:]))
...
class TrulyAdaptiveThresholdController:
    ...
    def _handle_discovery_phase(self, metrics: Dict[str, Any]):
        ...
        self.environment.add_discovery_sample(audio_level, 
                                            metrics.get('transcription_success', False))
    ...
    def _calculate_performance_score(self, transcription_success: bool, 
    ...
    def _adapt_based_on_performance(self) -> bool:
        ...
        if needs_adaptation:
            return self._apply_adaptive_change(adaptation_direction, adaptation_magnitude)
    ...
    def _apply_adaptive_change(self, direction: int, magnitude: float) -> bool:
        ...
        self.logger.info(f"Adaptive threshold change: {old_threshold:.6f} -> {self.current_threshold:.6f}")
...
class TrulyAutonomousAudioProcessor:
    ...
    def process_audio_chunk(self, chunk: np.ndarray) -> Tuple[bool, Optional[np.ndarray]]:
        ...
        with self._lock:
            ...
            self.signal_processor.update_signal_profile(enhanced_chunk, is_likely_speech)
            ...
            if is_likely_speech:
                if not self.is_speech_active:
                    ...
                    self.logger.debug(f"Speech detected: RMS={rms:.6f}, threshold={current_threshold:.6f}")
    ...
    def _finalize_speech_segment(self, current_time: float, current_rms: float) -> Tuple[bool, Optional[np.ndarray]]:
        ...
        self.logger.debug(f"Finalizing segment: {audio_duration:.1f}s, quality={quality_score:.3f}")
    ...
    def report_transcription_result(self, audio_duration: float, segments: List[str], 
                                  signal_quality: float, audio_rms: float):
        """Report results for adaptive learning"""
        transcription_success = len(segments) > 0 and any(len(s.strip()) > 1 for s in segments)
        
        # Detect likely false positives
        false_positive_likely = (
            not transcription_success and 
            audio_duration > 2.0 and 
            signal_quality < 0.3
        )
        ...
        adapted = self.threshold_controller.update_threshold_adaptively(performance_metrics)
    ...
    def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': self.successful_transcriptions / max(self.total_processing_attempts, 1),
...
def replace_with_truly_adaptive_processing(vocalflow_system):
    ...
    def truly_adaptive_audio_stream():
        ...
        logger.info("Truly adaptive audio processing started - discovering environment...")
        ...
        while vocalflow_system.is_running:
            try:
                ...
                should_process, audio_buffer = autonomous_processor.process_audio_chunk(chunk)
                
                if should_process and audio_buffer is not None:
                    # Transcribe
                    start_time = time.time()
                    segments = vocalflow_system.transcription_engine.transcribe_audio(audio_buffer)
                    ...
                    signal_quality = autonomous_processor.signal_processor.calculate_relative_quality(audio_buffer)
                    ...
                    logger.debug(f"Processed: {audio_duration:.1f}s -> {len(segments)} segments "
                    ...
                    if segments and signal_quality > 0.3:
                        for text in segments:
                            if vocalflow_system.filter.is_good_text(text):
                                corrected_text = vocalflow_system.filter.apply_corrections(text)
                                
                                # Handle commands
                                if (vocalflow_system.enable_commands and 
                                    vocalflow_system.linguistic_bridge and 
                                    vocalflow_system.command_agent):
                                    
                                    is_command, command_type, metadata = vocalflow_system.linguistic_bridge.process_command(corrected_text)
                                    
                                    if is_command:
                                        logger.info(f"Command: '{command_type}' from '{corrected_text}'")
                                        try:
                                            success = vocalflow_system.command_agent.process_text_enhanced(corrected_text)
                                            if not success:
                                                logger.warning(f"Command '{command_type}' failed")
                                        except Exception as cmd_error:
                                            logger.error(f"Command error: {cmd_error}")
                                        continue
                                
                                # Handle dictation
                                formatted_text = vocalflow_system._format_text(corrected_text)
                                vocalflow_system.output_handler.type_text(formatted_text)
