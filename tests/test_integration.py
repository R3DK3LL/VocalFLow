...
class IntegrationTestRunner:
    ...
    def test_complete_pipeline(self) -> Dict[str, Any]:
        ...
        for test in pipeline_tests:
            ...
            print(
                f"    Expected: {test['expected_type']}, Got: {'command' if command_executed else 'dictation'}"
            )
            print(
                f"    Context: {context_detected}, Suggestions: {len(result['suggestions'])}"
            )
            print(f"    Result: {'PASS' if success else 'FAIL'}")

        success_rate = sum(1 for r in pipeline_results if r["success"]) / len(
            pipeline_results
        )
        print(f"\nPipeline success rate: {success_rate:.1%}")

        return {"tests": pipeline_results, "success_rate": success_rate}
    ...
    def test_component_integration(self) -> Dict[str, Any]:
        ...
        agent_success = all(
            agent_test[key]
            for key in ["enhanced_mode", "linguistic_available", "commands_available"]
        )
        ...
        print(
            f"    Bridge recognizes command: {communication_test['bridge_recognizes']}"
        )
        ...
        overall_success = sum(1 for r in integration_results if r["success"]) / len(
            integration_results
        )
        print(f"\nComponent integration success rate: {overall_success:.1%}")

        return {"tests": integration_results, "success_rate": overall_success}
    ...
    def test_profile_system(self) -> Dict[str, Any]:
        ...
        profile_test = {
            ...
            "writable": (
                os.access(profile_dir, os.W_OK) if profile_dir.exists() else False
            ),
        }
        ...
        try:
            ...
            profile_loaded = (
                loaded_profile is not None and loaded_profile.name == test_profile_name
            )
            ...
            print(
                f"    Profile operations: {'PASS' if profile_ops_test['success'] else 'FAIL'}"
            )
        ...
        profile_success = sum(1 for r in profile_results if r["success"]) / len(
            profile_results
        )
        print(f"\nProfile system success rate: {profile_success:.1%}")

        return {"tests": profile_results, "success_rate": profile_success}
    ...
    def test_cross_platform_compatibility(self) -> Dict[str, Any]:
        ...
        for cmd in expected_commands:
            try:
                result = subprocess.run(
                    ["which", cmd] if current_os != "Windows" else ["where", cmd],
                    capture_output=True,
                    text=True,
                    timeout=5,
                )
            ...
            cmd_test = {"command": cmd, "platform": current_os, "available": available}
        ...
        print(
            f"    Browser discovery: {browser_test['discovered']} ({browser_test['executable']})"
        )
        ...
        print(
            f"    Terminal discovery: {terminal_test['discovered']} ({terminal_test['executable']})"
        )
        ...
        command_success = sum(
            1
            for r in cross_platform_results
            if r.get("available", r.get("discovered", False))
        )
    ...
    def test_error_handling(self) -> Dict[str, Any]:
        ...
        print(
            f"    Graceful degradation: {'PASS' if graceful_test['success'] else 'FAIL'}"
        )
        ...
        try:
            ...
            invalid_command_test = {
                ...
                "no_crash": True,
                "returns_false": result
                == False,  # Should return False for invalid commands
        ...
        print(
            f"    Invalid command handling: {'PASS' if invalid_command_test['success'] else 'FAIL'}"
        )
        ...
        print(
            f"    Empty input handling: {'PASS' if empty_test['success'] else 'FAIL'}"
        )

        self.results["error_handling"] = error_results

        error_success = sum(1 for r in error_results if r["success"]) / len(
            error_results
        )
        print(f"\nError handling success rate: {error_success:.1%}")

        return {"tests": error_results, "success_rate": error_success}
    ...
    def generate_integration_report(
        self, pipeline, components, profiles, platform, errors
    ):
