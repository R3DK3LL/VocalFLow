...
class RobustnessTestRunner:
    ...
    def test_edge_cases(self) -> Dict[str, Any]:
        ...
        edge_cases = [
            ...
            # Very long inputs
            {
                "input": "open browser " * 100,
                "description": "extremely long repeated command",
            },
            {
                "input": "This is a very long sentence that goes on and on and contains many words but should still be processed correctly by the system without causing any crashes or errors.",
            ...
            # Ambiguous inputs
            {
                "input": "maybe open browser if possible",
                "description": "tentative command",
            },
        ...
        for case in edge_cases:
            print(
                f"  Testing {case['description']}: '{case['input'][:50]}{'...' if len(case['input']) > 50 else ''}'"
            )
            ...
            try:
                ...
                print(
                    f"    Result: {'Command' if result['command_executed'] else 'Text'}, Time: {processing_time:.4f}s"
                )
        ...
        success_rate = sum(
            1 for r in edge_case_results if r["processed_successfully"]
        ) / len(edge_case_results)
        print(f"\nEdge case handling success rate: {success_rate:.1%}")

        return {"tests": edge_case_results, "success_rate": success_rate}
    ...
    def test_malformed_input(self) -> Dict[str, Any]:
        ...
        malformed_inputs = [
            ...
            {
                "input": "open browser'; DROP TABLE users; --",
                "description": "sql injection style",
            },
            {
                "input": "open browser<script>alert(1)</script>",
                "description": "html injection style",
            },
            # Command injection attempts
            {
                "input": "open browser && rm -rf /",
                "description": "command injection attempt",
            },
            {
                "input": "open browser | cat /etc/passwd",
            ...
            {
                "input": "open browser " + "A" * 500,
                "description": "command with long suffix",
            },
            # Unicode edge cases
        ...
        for case in malformed_inputs:
            ...
            try:
                ...
                print(
                    f"    Handled gracefully: {'Command' if result['command_executed'] else 'Text'}"
                )
        ...
        graceful_handling_rate = sum(
            1 for r in malformed_results if r["handled_gracefully"]
        ) / len(malformed_results)
    ...
    def test_resource_limits(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"    {rapid_calls} calls in {total_time:.3f}s, avg: {avg_time:.4f}s per call"
            )
        ...
        resource_success = sum(1 for r in resource_results if r["success"]) / len(
            resource_results
        )
        print(f"\nResource limits handling: {resource_success:.1%}")

        return {"tests": resource_results, "success_rate": resource_success}
    ...
    def test_file_system_robustness(self) -> Dict[str, Any]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                print(
                    f"    Read-only test: {'Attempted' if readonly_attempted else 'Skipped (permissions)'}"
                )
        ...
        return {"tests": file_results, "success_rate": file_success}
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        self.generate_robustness_report(
            edge_results, malformed_results, resource_results, file_results
        )
    ...
    def generate_robustness_report(self, edge, malformed, resource, file_sys):
        ...
        # Security assessment
        security_issues = sum(
            1 for r in malformed["tests"] if not r.get("no_security_risk", True)
        )
        print(
            f"Security Issues: {security_issues} (Good)"
            if security_issues == 0
            else f"Security Issues: {security_issues} (Review needed)"
        )
