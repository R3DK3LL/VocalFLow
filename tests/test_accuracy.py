...
class AccuracyTestRunner:
    ...
    def test_command_variations(self) -> Dict[str, Any]:
        ...
        for category, variations in command_variations.items():
            ...
            for variation in variations:
                is_command, command_type, metadata = self.bridge.process_command(variation)
                ...
                print(f"    '{variation}' -> {is_command} ({command_type}, conf: {confidence:.2f}, {method}) {'✓' if correct else '✗'}")
            ...
            print(f"    Category accuracy: {accuracy:.2%} ({category_correct}/{category_total})")
    ...
    def test_false_positives(self) -> Dict[str, Any]:
        ...
        for text in non_command_texts:
            ...
            if is_command:
                false_positives += 1
                print(f"    FALSE POSITIVE: '{text}' -> {command_type} (conf: {confidence:.2f})")
    ...
    def test_confidence_scoring(self) -> Dict[str, Any]:
        ...
        # Calculate averages
        avg_high = sum(confidence_results['high_confidence']) / len(confidence_results['high_confidence'])
        avg_medium = sum(confidence_results['medium_confidence']) / len(confidence_results['medium_confidence'])
        
        print(f"\nAverage high confidence: {avg_high:.3f}")
        print(f"Average medium confidence: {avg_medium:.3f}")
        print(f"Confidence differentiation: {'Good' if avg_high > avg_medium else 'Needs improvement'}")
    ...
    def test_edge_cases(self) -> Dict[str, Any]:
        ...
        for text in edge_cases:
            ...
            print(f"    '{text}' -> {is_command} ({command_type}, {confidence:.2f}, {method})")
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        self.generate_accuracy_report(variation_results, false_positive_results, confidence_results)
    ...
    def generate_accuracy_report(self, variations, false_positives, confidence):
        ...
        print(f"Confidence Scoring: {'Reliable' if confidence['differentiation'] else 'Needs Work'}")
