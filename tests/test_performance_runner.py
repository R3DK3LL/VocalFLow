...
class PerformanceTestRunner:
    ...
    def test_memory_usage(self) -> Dict[str, float]:
        ...
        print(f"  Initialization overhead: {after_init_memory - baseline_memory:.2f} MB")
        ...
        print(f"  Memory stability: {'Stable' if max_memory - baseline_memory < 50 else 'Concerning'}")
    ...
    def test_stress_conditions(self) -> Dict[str, Any]:
        ...
        def process_commands_batch(batch_size: int, batch_name: str):
            start_time = time.time()
            commands = ["open browser", "volume up", "take screenshot"] * (batch_size // 3)
        ...
        for batch_size in batch_sizes:
            ...
            print(f"  Batch {batch_size}: {batch_time:.2f}s total, {batch_time/batch_size:.4f}s per command")
    ...
    def generate_summary_report(self):
        ...
        avg_processing = sum(self.results['command_processing_times']) / len(self.results['command_processing_times'])
