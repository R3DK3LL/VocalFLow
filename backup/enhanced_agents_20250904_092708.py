        ...
        if context_scores:
            ...
            if context_scores[best_context] > 2:  # Minimum confidence threshold
                if best_context != self.current_context:
                    self.logger.info(
                        f"Context changed: {self.current_context} -> {best_context}"
                    )
...
class EnhancedVocalFlowAgent:
    ...
    def process_text_enhanced(self, text: str) -> bool:
        ...
        if LINGUISTIC_AVAILABLE and self.bridge:
            ...
            if is_command:
                ...
                print(
                    f"Enhanced match: '{command_type}' (confidence: {confidence:.2f}, method: {method})"
                )

                # Execute the enhanced command
                success = self._execute_enhanced_command(command_type, text, metadata)
                if success:
                    self.workflow_agent.log_activity(
                        f"command: {command_type}", current_context
                    )
                    return True
        ...
        if self.config.command_trigger.lower() in text.lower():
            clean_text = (
                text.lower().replace(self.config.command_trigger.lower(), "").strip()
            )
            ...
            for pattern, action in self.commands.items():
                ...
                if match:
                    try:
                        if callable(action):
                            if (
                                hasattr(action, "__code__")
                                and action.__code__.co_argcount > 1
                            ):
                                action(match)
                            else:
                                action()
                        ...
                        self.workflow_agent.log_activity(
                            f"command: {clean_text}", current_context
                        )
                        return True
    ...
    def _execute_enhanced_command(
        self, command_type: str, text: str, metadata: Dict
    ) -> bool:
    ...
    def _handle_volume_command(self, text: str) -> bool:
        ...
        if any(word in text_lower for word in ["up", "increase", "louder", "higher"]):
            ...
            word in text_lower for word in ["down", "decrease", "lower", "quieter"]
        ):
    ...
    def _volume_up(self) -> bool:
        ...
        try:
            ...
            try:
                subprocess.run(
                    ["amixer", "set", "Master", "10%+"], check=True, capture_output=True
                )
    ...
    def _handle_screenshot_command(self) -> bool:
        """Take a screenshot"""
        screenshots_dir = (
            f"/home/{os.getenv('USER', 'user')}/Pictures/VocalFlow_Screenshots"
        )
    ...
    def _new_file(self, match):
        ...
        editor = self._discover_application("code") or self._discover_application(
            "editor"
        )
        if editor:
            if file_type == "file":
...
class VocalFlowAgentSystem:
    ...
    def __init__(
        self, profile_name: str = "default", config: Optional[AgentConfig] = None
    ):
        self.config = config or AgentConfig()
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Enable workflow learning",
    )
    parser.add_argument(
        "--enable-proactive", action="store_true", help="Enable proactive suggestions"
    )
    ...
    try:
        ...
        print(
            "Features: Linguistic Commands, Context Awareness, Auto-completion, Workflow Learning"
        )
