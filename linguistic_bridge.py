...
try:
    ...
    print("Warning: Enhanced command matcher not available, falling back to basic matching")
...
class LinguisticBridge:
    ...
    def __init__(self, yaml_path: str = None, enable_enhanced: bool = True):
        ...
        if self.enable_enhanced:
            try:
                yaml_file = yaml_path or os.path.join('linguistic', 'command_variations.yaml')
    ...
    def _build_fallback_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            'browser_operations': [
                r'open\s+browser', r'launch\s+browser', r'start\s+browser',
                r'fire\s+up\s+browser', r'bring\s+up\s+browser'
            ],
            'volume_control': [
                r'volume\s+up', r'volume\s+down', r'mute', r'unmute',
                r'increase\s+volume', r'decrease\s+volume'
            ],
            'screenshot': [
                r'take\s+screenshot', r'capture\s+screen', r'screenshot',
                r'snap\s+screen', r'grab\s+screenshot'
            ],
            'terminal_operations': [
                r'open\s+terminal', r'launch\s+terminal', r'start\s+terminal',
                r'bring\s+up\s+terminal', r'fire\s+up\s+terminal'
            ]
        }
    ...
    def process_command(self, text: str) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if learning_suggestion:
            print(f"💡 Learning opportunity: '{text}' might be '{learning_suggestion['suggested_category']}' (confidence: {learning_suggestion['confidence']:.2f})")
    ...
    def _enhanced_process(self, text: str) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if match_result and match_result.get('confidence', 0) > 0.7:
            return True, match_result.get('command', ''), {
    ...
    def analyze_failed_command(self, text: str, alpha: float = 0.4) -> Optional[Dict]:
        ...
        valid_categories = set(self.fallback_patterns.keys())  # browser_operations,  volume_control, screenshot, terminal_operations
    ...
    def _extract_yaml_keywords(self, variations: List[str]) -> Set[str]:
        ...
        for variation in variations:
            ...
            keywords.update(word for word in words if word not in stopwords and len(word) > 2)
    ...
    def get_learning_stats(self) -> Dict[str, Any]:
        ...
        if yaml_categories:
            total_variations = sum(len(variations) for variations in yaml_categories.values())
            total_keywords = sum(len(self._extract_yaml_keywords(variations)) 
        ...
        return {
            ...
            'total_fallback_patterns': sum(len(patterns) for patterns in self.fallback_patterns.values()),
...
class AgentLinguisticIntegrator:
    ...
    def enhance_process_text(self, original_process_func, text: str) -> bool:
        ...
        if is_command:
            print(f"🎯 Linguistic match: {command_type} ({metadata.get('method', 'unknown')})")
    ...
    def _execute_mapped_command(self, command_type: str, original_text: str, metadata: Dict) -> bool:
...
if __name__ == "__main__":
    ...
    for cmd in known_commands:
        ...
        print(f"'{cmd}' -> Command: {is_cmd}, Type: {cmd_type}, Method: {metadata.get('method', 'none')}")
